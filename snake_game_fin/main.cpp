...
{
    mciSendString(_T("open ..//snake_file//bgm.mp3 alias bkmusic"), NULL, 0, NULL);//加载音乐
    mciSendString(_T("open ..//snake_file//out.mp3 alias outmusic"), NULL, 0, NULL);
    mciSendString(_T("open ..//snake_file//genshin.mp3 alias gsmusic"), NULL, 0, NULL);
...
{
    loadimage(&img_start,_T("..//snake_file//game_start.png"));
    loadimage(&img_login,_T("..//snake_file//game_login.png"));
    loadimage(&img_select_diff,_T("..//snake_file//game_select.jpg"));
    loadimage(&genshin,_T("..//snake_file//genshin.png"));
    loadimage(&white,_T("..//snake_file//white.png"));
}

void save_info(const string& username,const string& password)//注册时,将信息输入至txt文本文档中
{
    ofstream psFile(R"(../snake_file/login_information.txt)",ios::app);
...
bool read_info(const string& username,const string& password)//从txt文本文档中，验证登录信息
{
    ...
    ifstream prFile(R"(../snake_file/login_information.txt)");
...
void save_MaxScore()//一次游戏后，写入最大分数
{
    ...
    int ibuf = 0;
    FILE* pFile;
    pFile = fopen(R"(../snake_file/MaxScore_information.txt)","r");
    if(pFile == NULL){
    ...
    fclose(pFile);
    if(MaxScore > ibuf){
        pFile = fopen(R"(../snake_file/MaxScore_information.txt)","w");
        fprintf(pFile,"%d\n",MaxScore);
        fclose(pFile);
    }
    return;
